...
class OverlayUI {
  constructor() { this.el = null; this.isDragging = false; this.dragOffset = { x: 0, y: 0 }; }
  ...
  showResult(username, result, isSelf = false, oppName = '', myName = '') {
    ...
    this.el.innerHTML = `
      <div class="crs-header">
        <div class="crs-logo"><span class="crs-logo-icon">♟</span> CHESS RISK SCORE</div>
      ...
      <div class="crs-context-panel" id="crs-context-panel" style="display:none">
        <div class="crs-ctx-section">
          ...
          <div class="crs-ctx-formula">Risk = AgeFactor × (<span class="crs-ctx-w">0.35</span>·OWR + <span class="crs-ctx-w">0.30</span>·RWR + <span class="crs-ctx-w">0.35</span>·ACC) × RatingTrust</div>
          <div class="crs-ctx-legend">OWR = Overall Win Rate · RWR = Recent Win Rate · ACC = Accuracy Risk</div>
          <div class="crs-ctx-legend" style="margin-top:2px;font-size:8px;color:rgba(255,255,255,0.2)">Rating Trust: high-rated players with many games receive reduced risk (100% at &lt;1500, down to 20% at 2800+)</div>
        </div>
        <div class="crs-ctx-section">
        ...
        <div class="crs-ctx-section crs-ctx-weights">
          ...
          ${(()=>{
            ...
            <div class="crs-ctx-bar-legend" style="margin-top:3px">
              ...
              ${hasDamp?`<span style="color:#60a5fa" title="High rating (${breakdown.fmtRating}) with many games reduces risk — established players are less suspicious">× Rating Trust ${dampPct}% = ${score}</span>`:''}
