...
class RiskCalculator {
  ...
  // Rating-based dampener: reduces risk score for established high-rated players
  // At 1500 and below: no dampening (×1.0)
  // 1500–2000: slight dampening (×0.95–0.85)
  // 2000–2500: moderate (×0.85–0.5)
  // 2500+: heavy (×0.5–0.2)
  // Also factors in total games: more games = more dampening at high ratings
  _ratingDampener(rating, totalGames) {
    if (!rating || rating < 1500) return 1.0;

    let ratingFactor;
    if (rating < 2000) ratingFactor = 1.0 - ((rating - 1500) / 500) * 0.15;      // 1.0 → 0.85
    else if (rating < 2500) ratingFactor = 0.85 - ((rating - 2000) / 500) * 0.35; // 0.85 → 0.50
    else ratingFactor = Math.max(0.2, 0.50 - ((rating - 2500) / 500) * 0.30);     // 0.50 → 0.20

    // Games confidence: 1000+ games at this level = full dampening effect
    const gamesFactor = Math.min(1.0, (totalGames || 0) / 1000);

    // Blend: if few games, less dampening (could be a smurf); many games = full dampening
    return 1.0 - (1.0 - ratingFactor) * gamesFactor;
  }

  _scoreForFormat(fmt, stats, recentGames, username, ageFactor, bestRating, platform) {
    ...
    // Rating-based dampener: high-rated players with many games are less suspicious
    // A 60% win rate at 2800 is expected; at 800 it's suspicious
    const dampener = this._ratingDampener(fmtRating, rec?.total || 0);
  ...
  calculate(data, preferredFormat, platform = 'chesscom') {
    ...
    let bestBreakdown={overallWinrate:0,recentWinrate:0,accRisk:0,overallWinPct:0,recentWinPct:0,dampener:1};
    ...
    for (const fmt of formatsToScore) {
      const r = this._scoreForFormat(fmt, stats, recentGames, username, ageFactor, bestRating, platform);
      if (r.score > bestScore || formatsToScore.length === 1) {
        bestScore=r.score;
        bestBreakdown={overallWinrate:r.overallWR,recentWinrate:r.recentWR,accRisk:r.accRisk,overallWinPct:r.overallWinPct,recentWinPct:r.recentWinPct,dampener:r.dampener};
